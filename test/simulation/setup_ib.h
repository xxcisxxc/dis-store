...
void setup_ib(size_t buf_size, int is_server, char *server_name, char *sock_port)
{
    int ret;
...
int post_write_signaled(uint32_t offset)
{
    ...
    uint64_t raddr = ib_res.raddr + offset;
...
int post_read_signaled(uint32_t offset)
{
    uint32_t lkey = ib_res.mr->lkey;
    uint64_t wr_id = 200;
    struct ibv_qp *qp = ib_res.qp;
